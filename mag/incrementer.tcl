...
set REPEATS 10     ;# Repeats of the pair of cells
set ROW_HEIGHT 544
...
for {set i 0} {$i < $REPEATS} {incr i} {
    ...
    paint via1
    box 928 [expr $i * $ROW_HEIGHT + 480] 1150.5 [expr $i * $ROW_HEIGHT + 506]
    paint metal2
    ...
    paint via1
    box 928 [expr $i * $ROW_HEIGHT + 38] 1150.5 [expr $i * $ROW_HEIGHT + 64]
    paint metal2
    ...
    paint metal2
    box 882 [expr $i * $ROW_HEIGHT + 423] 920 [expr $i * $ROW_HEIGHT + 461]
    label q\[[expr 2 * $REPEATS - 2 - $i * 2]\] center metal2
    port make 
    port class output
    port use signal
    ...
    paint metal2
    box 882 [expr $i * $ROW_HEIGHT + 83] 920 [expr $i * $ROW_HEIGHT + 121]
    label q\[[expr 2 * $REPEATS - 1 - $i * 2]\] center metal2
    port make 
    port class output 
    port use signal
}
...
for {set i 0} {$i <= $REPEATS} {incr i} {
    ...
    if {$i == 0} {
        box 200 [expr $i * $ROW_HEIGHT - 17] 370 [expr $i * $ROW_HEIGHT + 117]
        paint metal3
        box 200 [expr $i * $ROW_HEIGHT + 100 - 17] 370 [expr $i * $ROW_HEIGHT + 117]
    } else {
        box 200 [expr $i * $ROW_HEIGHT - 17] 370 [expr $i * $ROW_HEIGHT + 17]
        paint metal3
    }
    label VPWR center metal3
    port make
    port class bidiretional
    port use power

    box 200 [expr $i * $ROW_HEIGHT - 17] 370 [expr $i * $ROW_HEIGHT + 17]
    paint metal2

    #box 0 [expr $i * $ROW_HEIGHT - 24] 960 [expr $i * $ROW_HEIGHT + 24]
    #label VPWR center metal1
    #port make
    #port class bidiretional
    #port use power
    ...
    #box 0 [expr $i * $ROW_HEIGHT - 24 + 272] 960 [expr $i * $ROW_HEIGHT + 24 + 272]
    #label VGND center metal1
    #port make
    #port class bidiretional
    #port use ground
...
# Hack
box 1370 5299 1450 5454
paint nwell

#move origin to -150 -50
move origin left 150 
move origin down 50
box 0 0 1550 [expr $REPEATS * $ROW_HEIGHT + $ROW_HALF_HEIGHT + 100]
property FIXED_BBOX 0 0 1550 [expr $REPEATS * $ROW_HEIGHT + $ROW_HALF_HEIGHT + 100]
save incrementer.mag
#flatten -dobox -dotoplabels incrementer
gds write ../macro/incrementer.gds
lef write ../macro/incrementer.lef -hide -pinonly
quit -noprompt
